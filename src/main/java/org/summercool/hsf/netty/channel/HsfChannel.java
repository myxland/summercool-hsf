...
import org.summercool.hsf.pojo.CallbackMessageWrapper;
...
public class HsfChannel implements Channel {
	...
	public void writeAsync(Object msg, final AsyncCallback<?> callback) {
		...
		if (callback != null) {
			...
			try {
				// 存储callback
				callbacks.put(request.getSeq(), callback);

				//
				Object cbMsg = getCallbackMessage(msg);
				callback.setParam(cbMsg);
				if (msg instanceof CallbackMessageWrapper) {
					msg = cbMsg;
				}
				...
				channelFuture.addListener(new ChannelFutureListener() {
					public void operationComplete(ChannelFuture future) throws Exception {
						if (!future.isSuccess()) {
							...
							callback.doExceptionCaught(future.getCause(), HsfChannel.this, callback.getParam());
						}
					}
				});
			} catch (Throwable e) {
	...
	private Object getCallbackMessage(Object msg) {
		...
		if (msg instanceof RemoteServiceObject) {
			...
			boolean holdCallbackMsg = LangUtil.parseBoolean(service.getOption(HsfOptions.HOLD_CALLBACK_MESSAGE), false);
			//
	...
	private ChannelFuture innerClose() {
		...
		for (AsyncCallback<?> callback : callbacks.values()) {
			callback.doExceptionCaught(new ClosedChannelException(), this, callback.getParam());
		}
